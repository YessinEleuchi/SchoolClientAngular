...
export class AdminManagementComponent implements OnInit {
  ...
  applyFilters(): void {
    ...
    if (this.selectedFilter !== null) {
      switch (this.selectedEntityType) {
        case 'field':
          filtered = filtered.filter(item => item.cycle?.id === this.selectedFilter);
          break;
        case 'specialization':
          filtered = filtered.filter(item => item.field?.id === this.selectedFilter);
          break;
        case 'level':
          filtered = filtered.filter(item => item.specialization?.id === this.selectedFilter);
          break;
        case 'group':
          filtered = filtered.filter(item => item.level?.id === this.selectedFilter);
          break;
        case 'subject':
          filtered = filtered.filter(item => item.level?.id === this.selectedFilter);
          break;
  ...
  getFilterOptions(): any[] {
    switch (this.selectedEntityType) {
      case 'field': return this.relatedData.cycles || [];
      case 'specialization': return this.relatedData.fields || [];
      case 'level': return this.relatedData.specializations || [];
      case 'group': return this.relatedData.levels || [];
      case 'subject': return this.relatedData.levels || [];
      default: return [];
    }
  }
  ...
  getFilterLabel(): string {
    switch (this.selectedEntityType) {
      case 'field': return 'Cycle';
      case 'specialization': return 'Field';
      case 'level': return 'Specialization';
      case 'group': return 'Level';
      case 'subject': return 'Level';
      default: return '';
    }
  }
}
